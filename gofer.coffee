do ( $ = jQuery ) ->
  ...
  # context we get for free. It's "this" as passed to $.fn.gofer.
  ...
  # handleClick() is bound to elements matching the selector on which gofer is called.
  ...
  goferOff = ->
    ...
    config.$window.off( "popstate.gofer" )    
  ...
  fnGofer = ( where, options, callback ) ->
    ...
    config.$body.on "click.gofer", "#{config.selector}", (event) ->
    ...
    # if we have multiple target areas, and thus multiple calls of gofer, we don't want to repeatedly attach events (thus repeatedly firing them onpopstate)
    # since all instances of gofer will be saving html to sessionStorage, we only need one popstate.
  ...
  # but occurs on the initial page that gofer is called on.
  storeThisPage = () ->
